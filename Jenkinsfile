    ...
    stage('deploy') {
        ...
        sshagent(credentials: ['04d588f3-fbbd-4547-8b0c-9c23de17e21e']) {
            if (buildPlatform.contains('darwin')) {
                ...
                sh 'ssh -i ' + sshKeyPath + ' -o StrictHostKeyChecking=no ' + nginxServerUsername + '@' + nginxServerIP + ' "cd ' +nginxProjectPath + ' && tar -zcvf ZSWKMAdmin\\`date -r ./ZSWKMAdmin +%Y%m%d%H%M%S%Z\\`.tar.gz -C ./ZSWKMAdmin dist && rm -rf ./ZSWKMAdmin/*"'
                sh 'ssh -i ' + sshKeyPath + ' -o StrictHostKeyChecking=no ' + nginxServerUsername + '@' + nginxServerIP + ' "cd ' +nginxProjectPath + ' && tar -zxvf ~/ZSWKMAdmin.tar.gz -C ./ZSWKMAdmin && rm -rf ~/ZSWKMAdmin.tar.gz"'
            } else if (buildPlatform.contains('linux')) {
                sh "ssh -o StrictHostKeyChecking=no ${nginxServerUsername}@${nginxServerIP} uname -a"
                sh "scp -o StrictHostKeyChecking=no ./ZSWKMAdmin.tar.gz ${nginxServerUsername}@${nginxServerIP}:~/"
                sh 'ssh -o StrictHostKeyChecking=no ' + nginxServerUsername + '@' + nginxServerIP + ' "cd ' +nginxProjectPath + ' && tar -zcvf ZSWKMAdmin\\`date -r ./ZSWKMAdmin +%Y%m%d%H%M%S%Z\\`.tar.gz -C ./ZSWKMAdmin www && rm -rf ./ZSWKMAdmin/*"'
